  ...
  where
    ...
    evenList count list =
      case list of
        [] -> []
        (x:xs) -> if even count
                    then x : evenList (count + 1) xs
                    else evenList (count + 1) xs
...
contains number list =
  case list of
    [] -> []
    (x : xs) -> if number `elem` x
                  then x : contains number xs
                  else contains number xs
...
rewind list =
  if null list
    then []
    else list !! lastIndex : rewind (take lastIndex list)
